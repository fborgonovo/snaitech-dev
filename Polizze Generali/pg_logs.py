...
def pg_get_console_handler():
   console_handler = logging.StreamHandler(sys.stdout)
   console_handler.setFormatter(FORMATTER)
   return console_handler

def pg_get_file_handler():
   file_handler = TimedRotatingFileHandler(c.LOGS_FN, when='midnight', delay=True)
   file_handler.setFormatter(FORMATTER)
   return file_handler

def pg_get_logger(logger_name):
   logger = logging.getLogger(logger_name)
   logger.setLevel(logging.DEBUG)
   logger.addHandler(pg_get_console_handler())
   logger.addHandler(pg_get_file_handler())
