<#
    ...
    v0.4  : Debug
    v0.5  : Added numeric code from custom attribute 6
    v0.6  : Debug
    v0.7a : Debug
    v0.7b : data_assunzione field must be '0'
            Employee data field does not need to be checked
            SP groups must be checked using the data lenght
            from the relative SQL
    v0.7c : Changed nested if construct with a switch in "### Ignore known duplicates"
            Commented out constants non yet in use
...
<# Local data will be used until exceptions have been corrected by HR #>
...
### Ignore known duplicates
    switch ($restData.samAccountName) {
        'system' {
            logger -Message "Known duplicate: $(restData.samAccountName)." -Level $severity[$infoIdx] -Path $severityFile[$infoIdx]
        ...
        default {
            logger -Message "Loading data for: $($restData.mail). Skipping." -Level $severity[$warningIdx] -Path $severityFile[$infoIdx]
