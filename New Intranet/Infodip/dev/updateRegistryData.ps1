﻿<#
    ...
    REST call:
        URL: "http://10.177.111.32:8080/Employee/getXMLForExcel?caller=intranet&firma=E2E38FDC09A1CAEF479F96E887D049DB&requestId=1"
        Parameters: intranet - - E2E38FDC09A1CAEF479F96E887D049DB 1
    ...
    Workflow:
        loadAncillaryTables
            ...
            unitaProduttiva
        Loading REST data
        Loading live SQL data
        delRemovedRecords
        foreach $record in $restData
            ### Temporary patch. Teleippica -> Epiqa
            ### Ignore known duplicates  ('system', 'sp_user_test')
            ### Verify that the user is not present in the DB
                Query returned no data. Adding new record
                Record found. Checking for changes
                Multiple records found. Skipping
        ### Archive logs
        ### Logs retention
...
{
    ### Make sure $data_assunzione and $data_nascita are blank
    $codAzienda, `
    $codDipendente, `
    $id_reparto, `
    $id_squadra, `
...
### Temporary patch. Epiqa -> Teleippica
            if ($existingMail -match "epiqa")
            {
                $existingMail = $existingMail.ToLower().Replace("epiqa", "teleippica")
            }
### End patch

#    $employeeMail = $restData | Select-XML -XPath "//xmlForExcel/xmlForExcel/mail[text()='$($existingMail)']"
            $employeesMail = $restData | Select-XML -XPath "//xmlForExcel/xmlForExcel/mail[text()]"

            $employeeMail = $null
            foreach ($mail in $employeesMail)
            {
                if ($existingMail -eq $mail)
                {
                    $employeeMail = $mail
                    break
                }
...
### Temporary patch. Teleippica -> Epiqa
            if ($existingMail -match "epiqa")
            {
                $employeeMail = $employeeMail -Replace "teleippica", "epiqa"
            }
### End patch
...
##        updateRecord $restData.mail.ToLower() $newValue
        logger -Message "[UPDATED] $($sqlData.ItemArray[$mail_idx].ToLower())"          -Level $severity[$infoIdx] -Path $changeLog[$sameIdx]
        logger -Message "-------------------------------------------------------------" -Level $severity[$infoIdx] -Path $changeLog[$updatedIdx]
    }
    else
    {
...
<#
    if ($restData.samAccountName -eq 'system')
    {
        $SqlCommand = "SELECT * FROM $($DIPENDENTIAD_TABLE) WHERE [mail] = '$($restData.mail)' AND [samAccountName] = 'borgonovo_furio'"
    }
    else
    {
        if ($restData.samAccountName -eq 'sp_user_test')
        {
            $SqlCommand = "SELECT * FROM $($DIPENDENTIAD_TABLE) WHERE [mail] = '$($restData.mail)' AND [samAccountName] = 'tringale_maurizio'"
        }
        else
        {
    ...
    #>
    switch ($restData.samAccountName) {
        'system' {
            logger -Message "Known duplicate: 'system'." -Level $severity[$infoIdx] -Path $severityFile[$infoIdx]
            $SqlCommand = "SELECT * FROM $($DIPENDENTIAD_TABLE) WHERE [mail] = '$($restData.mail)' AND [samAccountName] = 'borgonovo_furio'"
            break
        }
        'sp_user_test' {
            logger -Message "Known duplicate: 'sp_user_test'." -Level $severity[$infoIdx] -Path $severityFile[$infoIdx]
