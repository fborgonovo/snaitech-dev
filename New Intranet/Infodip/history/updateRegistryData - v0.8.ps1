<#
    ...
    v0.8 :  Changed SQL command building method with prepared statements.
#>
...
"@
    ...
    $SqlDataset = execSqlCommand $SqlCommand $SqlData
...
@"
        UPDATE
            @DIPENDENTIAD_TABLE
        SET
            @sqlColumnsName = @NewValue
        WHERE
            [mail] = @sqlDataKey
"@
    $SqlData =
        $DIPENDENTIAD_TABLE,
        $sqlColumnsName[$sqlColumnName_idx],
        $NewValue,
        $sqlDataKey
...
function checkSamExceptions ($mail2check)
{
    $samExceptions = @{
        ...
        'antonio.cirinna@snaitech.it' = 'cirinna_antonino'
...
### sede
    ...
    {
        $differ = $true
        $newValue = "'"+$($restData.streetAddress)+"'"
...
function execSqlCommand ($SqlCommand, $SqlData)
{
    ...
    try
    {
        ...
        $parametersName = $SqlCommand.Split('@') -replace "\r\n", "" -replace " ", "" -replace "," , "" -replace "VALUES\(" , "" -replace "\)", ""
        $i = 1
        ## Load data into SQL command
        foreach ($fieldData in $SqlData)
        {
            $paramName = "@" + $parametersName[$i]
            $i++
            $SqlCmd.Parameters.Add($paramName , $fieldData)
        }
...
foreach($restData in $restDataData.Risposta.xmlForExcel.xmlForExcel) {
    ...
    switch ($SqlDataset.Tables[0].Rows.Count) {
        ...
        default {
            logger -Message "Multiple records found for: $($restData.mail). Skipping." -Level $severity[$warningIdx] -Path $severityFile[$warningIdx]
