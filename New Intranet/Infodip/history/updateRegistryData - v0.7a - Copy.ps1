...
{
    logger -Message "*** Begin addRecord" -Level $severity[$infoIdx] -Path $severityFile[$addedIdx]
...
"@
	...
	logger -Message "Executing $($SqlCommand) command." -Level $severity[$infoIdx] -Path $severityFile[$addedIdx]
<# ###DEBUG###
	...
	if ([string]::IsNullOrEmpty($SqlDataset))
	{
        ...
        logger -Message "                $($SqlCommand)" -Level $severity[$infoIdx] -Path $changeLog[$errorIdx]
	}
	else
	{
...
#>
    logger -Message "*** End addRecord" -Level $severity[$infoIdx] -Path $severityFile[$addedIdx]
}

function updateRecord ($sqlDataKey, $sqlColumnName_idx, $NewValue, $oldValue)
{
    logger -Message "*** Begin updateRecord: $($sqlDataKey) - $($sqlColumnsName[$($sqlColumnName_idx)])" -Level $severity[$infoIdx] -Path $severityFile[$updatedIdx]

    $SqlCommand = "UPDATE $($DIPENDENTIAD_TABLE) SET [$($sqlColumnsName[$($sqlColumnName_idx)])] = $($NewValue) WHERE [mail] = '$sqlDataKey'"
	
	logger -Message "Executing $($SqlCommand) command." -Level $severity[$infoIdx] -Path $severityFile[$updatedIdx]
...
#>
    logger -Message "*** End updateRecord" -Level $severity[$infoIdx] -Path $severityFile[$updatedIdx]
}

function delRecord ($sqlDataKey)
{
    logger -Message "*** Begin delRecord" -Level $severity[$infoIdx] -Path $severityFile[$deletedIdx]

	$SqlCommand = "DELETE FROM $($DIPENDENTIAD_TABLE) WHERE [mail] = '$sqlDataKey'"
	
	logger -Message "Executing $($SqlCommand) command." -Level $severity[$infoIdx] -Path $severityFile[$deletedIdx]
...
#>
    logger -Message "*** End delRecord" -Level $severity[$infoIdx] -Path $severityFile[$deletedIdx]
}
...
### dataCreazione
    ...
    if ($differ)
    {
        ...
        logger -Message "-------------------------------------------------------------" -Level $severity[$infoIdx] -Path $changeLog[$infoIdx]
    }
    else
    {
