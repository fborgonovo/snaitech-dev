...
<#
    $a = $restDataData.Risposta.xmlForExcel.xmlForExcel

    foreach ($old_employee in $sqlData)
    {
        $deleted = $true
        foreach($employee in $restData)
        {
            if ($old_employee.mail -eq $employee.mail)
            {
                $deleted = $false
                break
            }
        ...
        if ($deleted)
        {
            delRecord $old_employee.mail.ToLower()
        }
    }
#>
...
### manager
    ...
    if ($restData.mail -eq 'manuela.palla@snaitech.it')
    {
        Write-Host 'check'
    }
...
### samResp
    ...
    {
        $differ = $true
        updateRecord $restData.mail.ToLower() $samResp_idx $samResp $sqlField
    }
...
#================================================
$SqlCommand = "SELECT * FROM [dipendenti].[test].[dipendentiAD]"
logger -Message "Executing $($SqlCommand) command." -Level $severity[$infoIdx] -Path $severityFile[$infoIdx]
...
#================================================
#delRemovedRecords $restDataData $SqlDataset.Tables[0].Rows
#exit
#================================================
